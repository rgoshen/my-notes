...
## Git Basics {collapsible="true" default-state="expanded"}
...
The following settings need to be configured for all commits:

- user.name
- user.email

## Getting Started with Git {collapsible="true" default-state="expanded"}
...
2. Convert the directory into repository `git init`
3. **ALT:** `git init <directory-name>` combines steps 1 & 2
4. Run git status `git status`
...
>
> > Another useful command to display more readable information on oneline with multiple branches execute: *
...
> If the number of commits increases beyond a certain count, use the
> **hashcode** command.
>
...
- each of the previous commits are also pointed to by a pointer called **HEADX** where X is an incremented number
  beginning with 1
...
## GitHub and Remotes {collapsible="true" default-state="expanded"}
...
3. Follow GitHub instructions
    ...
    2. Push from local repository to GitHub
...
Sometimes, files on origin (remote) may change without having come from main (local) so, you will need to pull from
origin to main.
...
- once the changes are pulled, the remote commit comes to the local and git will merge those changes in with the local
  changes
...
Scenario: `User A` wants to modify a particular line of a file in the local repo, `User B` wants to modify the same line
of
the same file in the remote repo

- `User A` attempts to pull the change, git shows a conflict and suggests to resolve the conflict manually
- To resolve conflicts manually **command:** `git merge`

## Branching Merging and Rebasing in Git {collapsible="true" default-state="expanded"}
...
1. reducing errors
2. protecting the content in the main repo
...
2. execute: **`git merge <branch name>`**
...
- When one branch has been created from a parent branch (in this case the main) and no more commits have been done to
  the parent branch
...
- When one branch has been created from a parent branch (in this case the main) and there have been more commits done to
  the parent branch since the creation of child branch
...
Two users change the same line in a file. One user wants to delete the line while the other just wants to modify the
line. Git does not have any idea how to merge these changes together, so it shows the conflict in the file.

**How does Git resolve these conflicts?**

Normally, if Git can resolve them it will do so automatically. However, in this scenario, it has no clue so it flags the
file and requests a manual resolve.
...
When you want to branch off from another commit (A) and start developing your own feature (B), it is likely your feature
is dependent on a subsequent commit done on another branch (main). What you do is get the last commit from that branch (
3). Git rebase will get used in this case.
...
Commits E & F will result in a conflict that can be resolved with either `git merge` or `git rebase`. Lets
try `git merge` first.
...
**Merge** will result in a new commit M which will inherit the changes from both commits. However, this will pollute the
history and make it confusing for other developers later on.
...
In **Rebase**, commit F will completely vanishes and its contents are available in commit R. This creates a clean,
straight history easily understood by future developers.
...
Clone takes two arguments **`<URL of the original repo>`** and **`<destination directory>`**. The duplicate repo is in
